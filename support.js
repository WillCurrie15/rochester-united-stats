...
(() => {
  ...
  var NUMBER_RE = /^-?\d+(\.\d+)?$/;
  var __exprFnCache = /* @__PURE__ */ new Map();
  function compileJsExpr(src) {
    let fn = __exprFnCache.get(src);
    if (!fn) {
      //! nosemgrep: eval-and-function-constructor
      fn = new Function("vals", "with (vals || {}) { return (" + src + "); }");
      __exprFnCache.set(src, fn);
    }
    return fn;
  }
  function resolve(vals, src) {
    ...
    if (expr.includes("=>")) {
      try {
        return compileJsExpr(expr)(vals);
      } catch (e) {
        console.warn("[dc-runtime] expr eval failed:", expr, e);
        return void 0;
      }
    }
